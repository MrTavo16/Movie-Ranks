...
const PostDetails = () => {
    ...
    return (<>
        {isLoaded && <div>
            ...
            <div id="main_post">
                <h2 style={{cursor:'pointer'}} onClick={()=>navigate(`/profile/${post.user_id}`)}>{post.username}</h2>
                ...
                {editingPost ? <div id="edit_and_cancel">
                    <div style={{cursor:'pointer'}} id="cancel_for_editing_post" onClick={() => {
                        setEditingPost(false)
                        setPostText('')
                    }}>Cancel</div>
                    <div style={{cursor:'pointer'}} id="edit_for_editing_post" onClick={handleEditPost}>Edit</div>
                </div> : <></>}
                <div style={{cursor:'pointer'}} id="main_likes_comments" onClick={(e) => e.preventDefault()}>
                    <div>
                        {post.users_liked && post.users_liked.includes(String(user.id)) ? <i style={{cursor:'pointer'}} className="fa-solid fa-thumbs-up" onClick={(e) => {
                            e.preventDefault()
                            dispatch(likePost({
                                id: post.id,
                    ...
                    <i style={{cursor:'pointer'}} className="fa-regular fa-comment comment_icon" onClick={() => {
                        setMainReply(true)
                    }}></i>
                    {user.id === post.user_id && !editingPost ? <div style={{cursor:'pointer'}} className="outer_edit" onClick={() => {
                        setEditingPost(true)
                        setPostText(post.post_text)
                    }}>Edit</div> : <></>}

                    {user.id === post.user_id ? <div style={{cursor:'pointer'}} className="outer_delete" onClick={handleDeletePost}>Delete</div> : <></>}
                </div>
            </div>
            <div>
                {nestedComs && nestedComs.map(ele => {
                    ...
                    return <div className="first_row_of_comments" key={ele.id}>
                        <h4 style={{cursor:'pointer'}} onClick={()=>navigate(`/profile/${ele.user_id}`)} className="username">{ele.username}</h4>
                        ...
                        {currCommentEditId === ele.id ? <div className="comment_edit_cancel">
                            ...
                            <div style={{cursor:'pointer'}} className="cancel_for_comment_editing" onClick={() => {
                                setCurrCommentEditId(0)
                                setCommentText('')
                            }}>Cancel</div>
                            <div style={{cursor:'pointer'}} className="edit_for_comment_editing" onClick={handleEditComment}>Edit</div>
                        </div> : <></>}
                        <div className="comment_likes_comments">
                            ...
                            <div style={{cursor:'pointer'}} className="reply" onClick={(e) => {
                                e.preventDefault()
                                setCurrCommentReplyId(ele.id)
                            }}>Reply</div>
                            {ele.user_id === user.id && !currCommentEditId ? <div style={{cursor:'pointer'}} className="outer_edit" onClick={() => {
                                setCurrCommentEditId(ele.id)
                                setCommentText(ele.comment_text)
                            }}>Edit</div> : <></>}
                            {(ele.user_id === user.id || user.id === post.user_id) && !currCommentEditId ? <div style={{cursor:'pointer'}} className="outer_delete" onClick={() => {
                        ...
                        <div>
                            {ele.children && ele.children.map(ele2 => {
                                ...
                                return <div className='second_row_of_comments' key={ele2.id}>
                                    <h4 style={{cursor:'pointer'}} onClick={()=>navigate(`/profile/${ele2.user_id}`)} className="username">{ele2.username}</h4>
                                    ...
                                    <div className="comment_likes_comments">
                                        ...
                                        <div style={{cursor:'pointer'}} className="reply" onClick={() => {
                                            setCurrCommentReplyId(ele2.id)
                                        }}>Reply</div>
                                        {ele2.user_id === user.id && !currCommentEditId ? <div style={{cursor:'pointer'}} className="outer_edit" onClick={() => {
                                            setCurrCommentEditId(ele2.id)
                                            setCommentText(ele2.comment_text)
                                        }}>Edit</div> : <></>}
                                        {(ele2.user_id === user.id || user.id === post.user_id) && !currCommentEditId ? <div style={{cursor:'pointer'}} className="outer_delete" onClick={() => {
                                    ...
                                    <div>
                                        {ele2.children && ele2.children.map(ele3 => {
                                            ...
                                            return <div className='third_row_of_comments' key={ele3.id}>
                                                <h4 style={{cursor:'pointer'}} onClick={()=>navigate(`/profile/${ele3.user_id}`)} className="username">{ele3.username}</h4>
                                                ...
                                                <div className="comment_likes_comments">
                                                    ...
                                                    {ele3.user_id === user.id && !currCommentEditId ? <div style={{cursor:'pointer'}} className="outer_edit" onClick={() => {
                                                        setCurrCommentEditId(ele3.id)
                                                        setCommentText(ele3.comment_text)
                                                    }}>Edit</div> : <></>}
                                                    {(ele3.user_id === user.id || user.id === post.user_id) && !currCommentEditId ? <div style={{cursor:'pointer'}} className="outer_delete" onClick={() => {
                                                    ...
                                                    {ele3.children && ele3.children.map(ele4 => {
                                                        ...
                                                        return <div className="fourth_row_of_comments" key={ele4.id}>
                                                            ...
                                                            <div className="comment_likes_comments">
                                                                ...
                                                                {ele4.user_id === user.id && !currCommentEditId ? <div style={{cursor:'pointer'}} className="outer_edit" onClick={() => {
                                                                    setCurrCommentEditId(ele4.id)
                                                                    setCommentText(ele4.comment_text)
                                                                }}>Edit</div> : <></>}
                                                                {(ele4.user_id === user.id || user.id === post.user_id) && !currCommentEditId ? <div style={{cursor:'pointer'}} className="outer_delete" onClick={() => {
                                                            ...
                                                            <div>
                                                                {ele4.children && ele4.children.map(ele5 => {
                                                                    ...
                                                                    return <div className="fifth_row_of_comments" key={ele5.id}>
                                                                        <h4 style={{cursor:'pointer'}} onClick={()=>navigate(`/profile/${ele5.user_id}`)} className="username">{ele5.username}</h4>
                                                                        ...
                                                                        <div className="comment_likes_comments">
                                                                            ...
                                                                            {ele5.user_id === user.id && !currCommentEditId ? <div style={{cursor:'pointer'}} className="outer_edit" onClick={() => {
                                                                                setCurrCommentEditId(ele5.id)
                                                                                setCommentText(ele5.comment_text)
                                                                            }}>Edit</div> : <></>}
                                                                            {(ele5.user_id === user.id || user.id === post.user_id) && !currCommentEditId ? <div style={{cursor:'pointer'}} className="outer_delete" onClick={() => {
            ...
            <div id="comment_text_bottom">
                ...
                {mainReply || editCom || currCommentReplyId ? <div style={{cursor:'pointer'}} className="reply_cancel" onClick={() => {
                    setCommentTextCreate('')
                    setCurrCommentEditId(0)
                    setCurrCommentReplyId(0)
                    setMainReply(false)
                ...
                {mainReply ? <div style={{cursor:'pointer'}} className="reply_post" onClick={handleReplyToMainPost}>Post</div> : <></>}
                {currCommentReplyId ? <div style={{cursor:'pointer'}} className="reply_post" onClick={handleCommentReplies}>Post</div> : <></>}
                </div>
